...
<div class="row" style="width: 80%; background: url('back12.jpg') no-repeat; background-size: cover; margin: auto;">
	<div class="col-md-8">
		...
		<?php
	session_start();
	$_SESSION["level2questions"]=array();
	include("dbconnection.php");
	$sql = "SELECT COUNT(DISTINCT `qno`) FROM `level2`";
	...
	$result=$connect->query($sql);
	$row=$result->fetch_assoc();
	$subject = $row["level2subject"];
	$_SESSION["subject2"]=$subject;
/*$username = $_SESSION["u"];*/
/*for ($i=0; $i<=5; $i++) {
   $num[$i] = rand(1,$max);
   for ($j=0; $j<$i; $j++) {             
...
sort($num); 
		...
		for($i=0; $i<5; $i++){
			array_push($_SESSION["level2questions"],$num[$i]);
			$sql = "SELECT * FROM `level2` WHERE `qno` = '$num[$i]'";
			$result=$connect->query($sql);
			$row=$result->fetch_assoc();
 ...
 ?>
	...
	<form name="level1ans" action="level2process.php" method="POST">
		<h3>Answer Sheet:</h3>
		<ul type="none">
			<li>
